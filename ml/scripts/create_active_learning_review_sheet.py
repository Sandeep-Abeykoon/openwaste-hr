...
def build_review_rows(
    ...
    for index, row in enumerate(selected_rows, start=1):
        review_rows.append(
            {
                "review_id": f"{review_prefix}_{index:04d}",
                "image_path": row["image_path"],
...
def build_summary(review_rows: list[dict[str, str]], sheet_name: str) -> dict[str, object]:
    return {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create active-learning human review sheet."
    )
    ...
    parser.add_argument("--review-prefix", type=str, required=True)
    parser.add_argument("--total-limit", type=int, default=100)
    parser.add_argument("--per-label-minimum-target", type=int, default=15)
