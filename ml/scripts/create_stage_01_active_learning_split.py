...
def main() -> None:
    stage_01_dir = Path("ml/data/manifests/stages/stage_01_trashnet")
    output_dir = Path("ml/data/manifests/stages/stage_01_trashnet_active_learning_v1")
    output_dir.mkdir(parents=True, exist_ok=True)

    base_train = read_csv(stage_01_dir / "known_train_v1.csv")
    base_val = read_csv(stage_01_dir / "known_val_v1.csv")
    base_test = read_csv(stage_01_dir / "known_test_v1.csv")
    unknown_val = read_csv(stage_01_dir / "unknown_val_v1.csv")
    unknown_test = read_csv(stage_01_dir / "unknown_test_v1.csv")

    reviewed_additions = read_csv(
        Path("ml/data/manifests/active_learning/stage_01_reviewed_known_additions_v1.csv")
    )
    ...
    summary = {
        "stage": "stage_01_trashnet_active_learning_v1",
        ...
        "rule": (
            "Stage 1 active-learning retraining uses original TrashNet training data "
            "plus only human-reviewed RealWaste known-class additions."
        ),
    }
