...
def build_known_addition_row(row: dict[str, str]) -> dict[str, str]:
    reviewed_label = normalize_text(row["reviewed_label"]).replace(" ", "_")
    ...
    return {
        ...
        "notes": (
            "Stage 1 active-learning human-reviewed known addition. "
...
def process_review_sheet(
    *,
    ...
    for row in rows:
        ...
        reviewed_label = normalize_text(row.get("reviewed_label", "")).replace(" ", "_")

        if review_status != "reviewed":
            invalid_or_pending.append(
                build_excluded_row(row, "not_reviewed_or_pending")
            )
            continue
        ...
        if reviewed_action == "add_to_training":
            ...
            known_additions.append(build_known_addition_row(row))
            continue

        if reviewed_action in {"exclude_or_unknown", "exclude", "unknown", "manual_review_only"}:
            excluded_or_unknown.append(
                build_excluded_row(row, "reviewed_as_exclude_or_unknown")
            )
            continue

        invalid_or_pending.append(
            build_excluded_row(row, "invalid_or_missing_reviewed_action")
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Process Stage 1 active-learning human review sheet."
    )
    ...
    parser.add_argument(
        "--known-output-csv",
        type=Path,
        default=Path("ml/data/manifests/active_learning/stage_01_reviewed_known_additions_v1.csv"),
    )

    parser.add_argument(
        "--excluded-output-csv",
        type=Path,
        default=Path("ml/data/manifests/active_learning/stage_01_reviewed_excluded_or_unknown_v1.csv"),
    )

    parser.add_argument(
        "--summary-json",
        type=Path,
        default=Path("ml/data/manifests/active_learning/stage_01_reviewed_label_summary_v1.json"),
    )
