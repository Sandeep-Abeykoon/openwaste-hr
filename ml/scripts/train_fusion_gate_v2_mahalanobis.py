...
def merge_key(row: dict[str, Any]) -> tuple[str, str]:
    return (
        ...
        normalize_image_path(row.get("image_path", "")),
    )
...
def merge_rows(
    *,
    ...
    mahalanobis_rows: list[dict[str, str]],
) -> list[dict[str, Any]]:
    temperature_lookup = {
        merge_key(row): row
    ...
    mahalanobis_lookup = {
        merge_key(row): row
        for row in mahalanobis_rows
    }

    merged_rows: list[dict[str, Any]] = []
    missing_temperature = 0
    missing_mahalanobis = 0

    for row in normal_rows:
        merged: dict[str, Any] = dict(row)

        temp_row = temperature_lookup.get(merge_key(row))
        if temp_row is None:
        ...
        maha_row = mahalanobis_lookup.get(merge_key(row))
        if maha_row is None:
            missing_mahalanobis += 1
            merged["mahalanobis_knownness"] = ""
            merged["mahalanobis_min_distance"] = ""
            merged["mahalanobis_nearest_class"] = ""
        else:
            merged["mahalanobis_knownness"] = maha_row.get("mahalanobis_knownness", "")
            merged["mahalanobis_min_distance"] = maha_row.get("mahalanobis_min_distance", "")
            merged["mahalanobis_nearest_class"] = maha_row.get("mahalanobis_nearest_class", "")

        merged_rows.append(merged)

    if missing_temperature:
        print(f"Warning: missing temperature rows: {missing_temperature}", flush=True)

    if missing_mahalanobis:
        print(f"Warning: missing Mahalanobis rows: {missing_mahalanobis}", flush=True)

    return merged_rows


def find_probability_columns(rows: list[dict[str, Any]]) -> list[str]:
    if not rows:
        return []
...
def add_derived_features(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for row in rows:
        ...
        if probability_columns:
            ...
            if total > 0:
                ...
                probs = np.ones(len(KNOWN_CLASS_NAMES), dtype=float) / len(KNOWN_CLASS_NAMES)

        else:
            confidence = min(max(to_float(row.get("confidence", "0")), 0.0), 1.0)
            other = (1.0 - confidence) / (len(KNOWN_CLASS_NAMES) - 1)
            probs = np.array([confidence] + [other] * (len(KNOWN_CLASS_NAMES) - 1), dtype=float)
        ...
        updated["softmax_entropy"] = float(-np.sum(probs * np.log(np.clip(probs, 1e-12, 1.0))))
    ...
    metadata = {
        ...
        "mahalanobis_feature": "mahalanobis_knownness",
    }
    ...
    *,
    ...
    for row in rows:
        ...
        for feature_name in feature_names:
            values.append(to_float(row.get(feature_name, "0")))
...
def add_fusion_predictions(
    *,
    ...
    for row, score in zip(rows, scores):
        ...
        updated["fusion_v2_knownness_score"] = float(score)
        updated["fusion_v2_threshold"] = float(threshold)
        updated["fusion_v2_accepted_as_known"] = accepted
        updated["fusion_v2_decision"] = (
            "known_fine_label" if accepted else "unknown_manual_review"
        )

        if accepted:
            updated["fusion_v2_user_visible_label"] = updated.get("pred_label", "")
            updated["fusion_v2_reject_reason"] = ""
        else:
            updated["fusion_v2_user_visible_label"] = ""
            updated["fusion_v2_reject_reason"] = "fusion_v2_knownness_below_threshold"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train Fusion Gate v2 with Mahalanobis feature-distance."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("ml/outputs/fusion_gate/stage_04_fusion_gate_v2_mahalanobis"),
    )
...
def main() -> None:
    ...
    normal_rows = read_csv(args.open_set_dir / "open_set_predictions_v1.csv")
    temperature_rows = read_csv(args.open_set_dir / "temperature_scaled_predictions_v1.csv")
    mahalanobis_rows = read_csv(args.mahalanobis_dir / "mahalanobis_scores_v1.csv")

    merged_rows = merge_rows(
        normal_rows=normal_rows,
        temperature_rows=temperature_rows,
        mahalanobis_rows=mahalanobis_rows,
    )
    ...
    x_known_val = build_feature_matrix(rows=known_val_rows, feature_names=FEATURES_V2)
    x_unknown_val = build_feature_matrix(rows=unknown_val_rows, feature_names=FEATURES_V2)
    x_known_test = build_feature_matrix(rows=known_test_rows, feature_names=FEATURES_V2)
    x_unknown_test = build_feature_matrix(rows=unknown_test_rows, feature_names=FEATURES_V2)
    ...
    validation_result["auroc_known_vs_unknown"] = float(
        roc_auc_score(
    ...
    test_result["auroc_known_vs_unknown"] = float(
        roc_auc_score(
    ...
    x_all = build_feature_matrix(rows=feature_rows, feature_names=FEATURES_V2)
    all_scores = fusion_gate.predict_proba(x_all)[:, 1]

    prediction_rows = add_fusion_predictions(
        rows=feature_rows,
        scores=all_scores,
        threshold=threshold,
    )
    ...
    model_path = args.output_dir / "fusion_gate_v2_mahalanobis_model.joblib"

    joblib.dump(fusion_gate, model_path)

    write_csv(args.output_dir / "fusion_gate_v2_predictions.csv", prediction_rows)
    write_csv(args.output_dir / "fusion_gate_v2_c_tuning.csv", tuning["rows"])

    metric_rows = [
        {
            "method": "fusion_gate_v2_mahalanobis",
            "split": "validation",
            **validation_result,
        },
        {
    ...
    write_csv(args.output_dir / "fusion_gate_v2_metrics.csv", metric_rows)

    summary = {
        "experiment": "stage_04_fusion_gate_v2_mahalanobis",
        "base_model": "stage_04_add_trashbox_clean_v1",
        "fusion_model": "LogisticRegression_with_StandardScaler",
        "feature_names": FEATURES_V2,
        ...
        "output_files": {
            "summary": str(args.output_dir / "fusion_gate_v2_summary.json"),
            "metrics": str(args.output_dir / "fusion_gate_v2_metrics.csv"),
            "predictions": str(args.output_dir / "fusion_gate_v2_predictions.csv"),
            "c_tuning": str(args.output_dir / "fusion_gate_v2_c_tuning.csv"),
            "model": str(model_path),
        },
        "rule": (
            "Fusion Gate v2 is trained using known_val + unknown_val only. "
            "Mahalanobis statistics were fitted using known_train only. "
            "Threshold is selected on validation only. "
            "Final reporting uses known_test + unknown_test."
        ),
    }

    (args.output_dir / "fusion_gate_v2_summary.json").write_text(
